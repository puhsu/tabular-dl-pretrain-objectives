        ...
        while eval_batch_size:
            try:
                predictions[part] = (
                    torch.cat(
                        [
                            apply_finetuned_model(part, idx)[1]
...
def apply_finetuned_model(part, idx):
    if bins_store:
    ...
    return (x_num, None), model(x_num, x_cat)[1].squeeze(-1)
...
for batch_idx in tqdm(stream_iterations(C.pretrain.n_iterations)):
    ...
    if stream.iteration % C.pretrain.validate_every == 0:
        ...
        if C.pretrain.early_stop_type == 'finetune':
            finetune_metrics, predictions = evaluate_finetune(['train', 'val', 'test'])
