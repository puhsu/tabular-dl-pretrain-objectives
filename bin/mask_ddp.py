    ...
    def evaluate(parts):
        ...
        for part in parts:
            ...
            if dist.is_initialized():
                ...
                predictions_l = list(torch.empty(world_size, p.size(0), p.size(1), dtype=p.dtype, device=p.device).unbind(0))
                ...
                predictions = torch.empty(D.size(part), predictions_.size(1), dtype=predictions_.dtype, device=predictions_.device)
                predictions.index_copy_(0, indexes, predictions_)
