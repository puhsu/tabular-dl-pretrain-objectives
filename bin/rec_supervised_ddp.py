...
def train(
    ...
    def evaluate(parts):
        ...
        for part in parts:
            ...
            pretrain_loss = mean(pretrain_losses)
            supervised_loss = mean(supervised_losses)

            if dist.is_initialized():
                pretrain_loss = torch.tensor([pretrain_loss], device=device)
                dist.all_reduce(pretrain_loss)
                pretrain_loss = pretrain_loss.item() / lib.get_world_size()

                supervised_loss = torch.tensor([supervised_loss], device=device)
                dist.all_reduce(supervised_loss)
                supervised_loss = supervised_loss.item() / lib.get_world_size()

            metrics[part] = {"pretrain_loss": pretrain_loss,
                             "supervised_loss": supervised_loss,
                             "pretrain_loss": pretrain_loss + supervised_loss}
    ...
    def loss_fn(pred_num, pred_cat, pred_y, target_num, target_cat, target_y, mask_num, mask_cat):
        return pretrain_loss_fn(pred_num, pred_cat, target_num, target_cat, mask_num, mask_cat) + supervised_loss_fn(pred_y, target_y)
