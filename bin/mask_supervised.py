...
class PretrainModel(nn.Module):
    def __init__(self):
        ...
        insert_pre_target = (C.pretrain.use_target and C.pretrain.target_injection_type == 'pre')
        self.target_embeddings = None
        if insert_pre_target:
            if D.is_regression:
                self.target_embeddings = lib.NumEmbeddings(
                    C.model.num_embedding_arch,
                    1,
                    d_num_embedding,
                    d_feature=None,
                ...
                self.target_embeddings = rtdl.CategoricalFeatureTokenizer(
                    [n_classes], d_num_embedding, True, 'uniform'
                )
...
def evaluate(parts):
    ...
    for part in parts:
        while eval_batch_size:
            try:
                ...
                for idx in dataloader:
                    ...
                    pretrain_predictions.append(cur_pretrain_predictions)
                    supervised_predictions.append(cur_supervised_predictions)
                pretrain_predictions = torch.cat(pretrain_predictions)
                supervised_predictions = torch.cat(supervised_predictions).squeeze(-1)
        ...
        hard_predictions = torch.zeros_like(pretrain_predictions, dtype=torch.long)
        hard_predictions[pretrain_predictions > 0] = 1
        features_accuracy = (hard_predictions.bool() == masks).sum(0) / hard_predictions.shape[0]
        pretrain_loss = pretrain_loss_fn(pretrain_predictions, masks).item()
        supervised_loss = supervised_loss_fn(supervised_predictions, Y[part]).item()
...
def loss_fn(pretrain_predictions, supervised_predictions, masks, target):
    ...
    return 2 * ((1 - C.pretrain.supervised_weight) * pretrain_loss + C.pretrain.supervised_weight * supervised_loss)
