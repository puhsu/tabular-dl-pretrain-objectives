    ...
    def __init__(self):
        ...
        if C.pretrain.use_target:
            if C.model.kind == "transformer":
                self.head = TargetCLSHead(d_in=self.backbone.d,
    ...
    def forward(self, x_num, x_cat, y=None):
        ...
        if self.target_embeddings is not None:
            ...
            if not D.is_regression:
                y = y.long()
            y = self.target_embeddings(y[:, None])
            inputs.append(y)
        ...
        h = self.backbone(x)
        return self.mask_head(h), self.ae_head(h)
...
def evaluate(parts):
    ...
    for part in parts:
        ...
        while eval_batch_size:
            try:
                for idx in zero.data.IndexLoader(
                    ...
                    mask_predictions, (pred_num, pred_cat) = apply_model(part, idx)
                    masks = get_masks('train', idx)
                    ...
                    if pred_cat:
                        ...
                        target_cat = None
                        mask_cat = None
                    mask_pretrain_losses.append(mask_loss_fn(mask_predictions, masks).item())
                    ae_pretrain_losses.append(ae_loss_fn(pred_num, pred_cat, target_num, target_cat, mask_num, mask_cat).item())
        ...
        ae_pretrain_loss = mean(ae_pretrain_losses)
        mask_pretrain_loss = mean(mask_pretrain_losses)
        metrics[part] = {"ae_pretrain_loss": ae_pretrain_loss,
                         "mask_pretrain_loss": mask_pretrain_loss,
                         "pretrain_loss": mask_pretrain_loss + ae_pretrain_loss}
...
def step_fn(idx):
    mask_predictions, (pred_num, pred_cat) = apply_model("train", idx)
    masks = get_masks("train", idx)
    ...
    if pred_cat:
        ...
        target_cat = None
        mask_cat = None
    return mask_predictions, masks, pred_num, pred_cat, target_num, target_cat, mask_num, mask_cat


def ae_loss_fn(pred_num, pred_cat, target_num, target_cat, mask_num, mask_cat):
...
def mask_loss_fn(predictions, masks):
    return F.binary_cross_entropy_with_logits(predictions, masks.float())


def loss_fn(mask_predictions, masks, pred_num, pred_cat, target_num, target_cat, mask_num, mask_cat):
    return mask_loss_fn(mask_predictions, masks) + ae_loss_fn(pred_num, pred_cat, target_num, target_cat, mask_num, mask_cat)
        ...
        if C.pretrain.early_stop_type == 'finetune':
            ...
            report.setdefault("metrics", OrderedDict()).setdefault("iteration_scores", OrderedDict())[stream.iteration] = {
                part: {
                    ...
                    "ae_pretrain_loss": metrics[part]["ae_pretrain_loss"],
                    "mask_pretrain_loss": metrics[part]["mask_pretrain_loss"],
