...
class Dataset:
    ...
    def __new__(cls, X_num=None, X_cat=None, y=None, y_info=None, task_type=None, n_classes=None, metric=None):
        inst = super().__new__(cls)
        inst.metric = metric
        return inst

    @classmethod
    def from_dir(cls, dir_: Union[Path, str], parts: List[str] = ['train', 'val', 'test']) -> 'Dataset':
        dir_ = Path(dir_)

        def load(item) -> ArrayDict:
            return {
                x: cast(np.ndarray, np.load(dir_ / f'{item}_{x}.npy')) if dir_.joinpath(f'{item}_{x}.npy').exists() else None # type: ignore[code]
                for x in parts
            }
    ...
    def calculate_metrics(
        ...
        metrics = {
            x: calculate_metrics_(
                self.y[x], predictions[x], self.task_type, prediction_type, self.y_info, self.metric
            )
        ...
        if self.task_type == TaskType.REGRESSION:
            ...
            if self.metric == 'logloss':
                ...
                score_key = 'roc_auc'
                score_sign = 1
            elif self.metric == 'amex':
                score_key = 'amex'
                score_sign = 1
            else:
                raise ValueError(f"Metric {self.metric} is not implemented")
        else:
            score_key = 'accuracy'
            score_sign = 1
...
def transform_dataset(
    ...
    dataset = replace(dataset, X_num=X_num, X_cat=X_cat, y=y, y_info=y_info, metric=metric)
...
def build_dataset(
    path: Union[str, Path], transformations: Transformations, cache: bool, parts: List[str] = ['train', 'val', 'test']
) -> Dataset:
    path = Path(path)
    dataset = Dataset.from_dir(path, parts)
...
def prepare_tensors(
    ...
    X_num, X_cat, Y = (
        None if x is None else {k: None if v is None else torch.as_tensor(v) for k, v in x.items()}
        for x in [dataset.X_num, dataset.X_cat, dataset.y]
    )
    if device.type != 'cpu':
        X_num, X_cat, Y = (
            None if x is None else {k: None if v is None else v.to(device) for k, v in x.items()}
            for x in [X_num, X_cat, Y]
        )
    assert X_num is not None
    assert Y is not None
    if not dataset.is_multiclass:
        Y = {k: None if v is None else v.float() for k, v in Y.items()}
...
class IndexLoader:
    def __init__(
            self, size: int, batch_size: int, shuffle=True, ddp=False,
    ) -> None:
        assert size > 0
        self._batch_size = batch_size
        self._shuffle = shuffle
        self._dataset = zero.data._IndicesDataset(size)

        if ddp:
            sampler = torch.utils.data.DistributedSampler(self._dataset, shuffle=shuffle)
            shuffle = None
        else:
            sampler = None
        ...
        self._epoch = 0
        self._loader = torch.utils.data.DataLoader(self._dataset, self._batch_size, sampler=sampler, shuffle=shuffle)
        self._device = lib.get_device()
