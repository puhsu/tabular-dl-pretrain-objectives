...
def get_device() -> torch.device:
    return torch.device(f'cuda:{get_rank()}' if os.environ.get('CUDA_VISIBLE_DEVICES') else 'cpu')
...
def backup_output(output_dir: Path, force=False) -> None:
    ...
    if _LAST_SNAPSHOT_TIME is None or time.time() - _LAST_SNAPSHOT_TIME > 10 * 60 or force:
...
def from_dict(datacls: type[T], data: dict) -> T:
    ...
    for field in fields(datacls):
        if is_dataclass(field.type):
            data[field.name] = from_dict(field.type, data.get(field.name, {}))
    return datacls(**data)
...
def setup_distributed_logs(is_master):
